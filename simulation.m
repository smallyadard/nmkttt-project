...
function results = simulation(params, output_filename)
    ...
    parfor rep=1:params.repetitions
        ...
        for p_i = 1:length(params.P_comm_ratio)
            ...
            %% JSC (FIREFLY ALGORITHM) ; thay opt_jsc_FA thành opt_jsc_FA_hybrid để sử dụng biến thể
            sens_streams = 1;
            % Gọi hàm FA mới tạo
            [Q_jsc, feasible, F_jsc_SSNR] = opt_jsc_FA(H_comm, params.sigmasq_ue, SINR_min_SOCP_NS, sensing_beamsteering, sens_streams, params.sigmasq_radar_rcs, params.P);
